...
let seleccionDePelicula = parseInt(prompt("Elija la pelicula que desea ver:\n" +`1_`+ pelicula1 + `\n` +`2_`+ pelicula2 + `\n`+`3_` + pelicula3 + `\n`+`4_`+ pelicula4 + `\n` +`9__SALIR`));

if (seleccionDePelicula == 1) {
    alert(`usted selecciono: `+ pelicula1)
    totalEntradas = parseInt(prompt(`El valor de la entrada es $`+ precioEntrada +` Cuantas entradas desea?`));
    precioTotalCompra = alert(`usted selecciono ` + totalEntradas + ` con un valor total de $`+multiplicacion(precioEntrada,totalEntradas));
...
while (nombre == "" || apellido == "" || isNaN(edad) || isNaN(pago) || isNaN(vencimiento) || isNaN(ccv)) {
...
alert("sus datos han sido ingresado correctamente \n" + nombre+` `+apellido+`\n`+`Esperamos que disfute su funcion`);
